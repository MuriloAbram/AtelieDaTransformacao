...
namespace AtelieDaTransformacao.Desktop.Services
{
    ...
    public class AuthApiService
    {
        ...
        /// </summary>
        public async Task<(bool Sucesso, UserDto? User, string ErrorMessage)> LoginAsync(
            string email,
            string password)
        {
            ...
            var (sucesso, data, error) =
                await _http.PostAsync<UserDto>(
                    "/api/auth/login",
                    loginDto);
        ...
        {
            return await _http.GetAsync<UserDto>(
                "/api/auth/me");
        }
